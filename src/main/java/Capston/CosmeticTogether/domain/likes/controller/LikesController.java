...
public class LikesController {
    ...
    public ResponseEntity<ResponseMessage> likeOrUnlikeBoard(@PathVariable("boardId") Long boardId) {
        boolean isValid = likeService.likeOrUnlikeBoard(boardId);
        if(isValid) {
            return ResponseEntity.ok(new ResponseMessage(HttpStatus.OK.value(), "게시글이 좋아요 목록에 추가되었습니다"));
        } else {
            return ResponseEntity.ok(new ResponseMessage(HttpStatus.OK.value(), "게시글이 좋아요 목록에 삭제되었습니다"));
        }
    }
}
